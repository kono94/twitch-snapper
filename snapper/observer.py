...
class KeywordData:
    ...
    def __str__(self) -> str:
        return f"count: {self.count} \t isActive: {self.is_active} \t active_intervals: {self.active_intervals} \t timestamp_activated: {self.timestamp_activated}"
...
class StreamObserver:
    ...
    def __init__(self, twitch_channel_name: str, twitch: Twitch):
        ...
        self.keyword_list = ["LUL", "KEKW", "Pog"]
        self.PAUSE_INTERVAL = 3
        self.ACTIVATION_THRESHOLD = 4
        self.ACTIVATION_INTERVAL = 15
        assert (
            self.ACTIVATION_INTERVAL >= self.PAUSE_INTERVAL
        ), "ACTIVATION_INTERVAL has to be >= than PAUSE_INTERVAL"
        assert (
            self.ACTIVATION_INTERVAL % self.PAUSE_INTERVAL == 0
    ...
    async def start_observing(self) -> None:
        irc_client = IRCClient.from_channel_name_only(
            self.twitch_channel_name, self.msg_queue
        )
            ...
            for keyword, keyword_data in self.keyword_count.items():
                ...
                if not keyword_data.is_active:
                    ...
                    if keyword_data.count >= 3 and self._is_trigger_ready_again():
                        Log.info(colored_string(f"Activate {keyword}!", Color.YELLOW))
                ...
                # Keyword is already active
                    ...
                    # Counted amount of keywords for a longer time after the initial activation, this time ended
                    if (
                        ...
                        Log.info(
                            colored_string(
                                f"After {pause * keyword_data.active_intervals} seconds, \
    ...
    async def _create_clip(self) -> str | None:
        """
        Uses the twitchAPI to create a clip right after the moment the special event was
        notices by the trigger logic.

        Returns:
            str | None: The ID of the clip or None if an exception happend. Mainly due to API errors
                        if there no broadcast_id can be found, the twitch clip creation failed, or the file could
                        not be created
        """
        ...
        try:
            ...
            #     f"{time.time()}_{self.twitch_channel_name}_ {createdClip.id}.txt", "w"
            # ) as file:
            #     await file.write(createdClip.id)
        ...
        # if clip:
        #     Log.info(f"Successfully created clip! Clip \n {clip_to_string(clip)}")
        # else:
        #     Log.error("Clip could not be created")
